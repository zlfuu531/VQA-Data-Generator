...
请基于提供的金融图像（可能是**统计图表**如折线图、柱状图、饼图、雷达图等，也可能是**纯报表**如财务报表、数据表格、股权结构表等），生成一道【{question_type_name_cn}】，用于评估大模型在金融图像精准定位与理解方面的能力。

**【总体原则：宁缺毋滥】**

- 你只需要为每张图片构造**少量但高质量、高区分度**的问题。
- 如果图像信息有限，宁可只提出 1-2 道非常好的题目，也不要为了数量牺牲质量。
- 每一道题都要有明确的考察点和清晰的解题路径，而不是简单读数题或单步查读。

**【任务要求】**

1. **必须生成需要多步推理的金融推理问题**，问题的答案只能依赖图像中的内容得到。可以包括但不限于：
   - **财务指标推理**（可计算或逻辑推断均可）：连乘/连加计算、权益比例拆解、年化收益率推算、加权平均、增长率计算等
   - **图表趋势分析与推断**：时间序列的趋势分析、极值比较、拐点识别、周期性判断等
   - **结构化图形推理**（如股权结构、资产结构、业务构成）：层级关系、持股比例、间接持股计算等
   - **多实体关系推断**：比较、排序、关联分析、差值或比值运算等
   - **比较类问题**：增长最快、占比最高、结构变化、相对表现等
   - **经济或业务含义推断**（必须基于图中客观信息）：风险收益特征、风险溢价变化等
   - **跨图表/跨表格综合分析**：多项指标之间的链式推理（如 A→B→C）

2. **问题必须属于金融领域的实际子任务之一**，如：
   - 公司金融：财务报表分析、利润结构、资产负债表项目推算
   - 行业分析：行业指标比较、市场份额变化、竞争格局
   - 股票估值：估值指标计算、财务比率推导
   - 风险管理：风险指标识别、波动率比较
   - 资产管理：资产配置、投资组合表现
...
3. **所有问题必须满足**：
   ...
   - 题干不允许直接给出图片内已有的数值、结构化信息或现成结论，只能引用图题、图例名称、科目名称、时间点/类别名称等作为线索
   - 不允许主观类问题（如"你怎么看""是否合理"）
   - 不允许开放式回答（如"列出可能的原因"）

4. **必须避免简单查读，要求多步推理**：
   - **针对统计图表**：不能是"直接从图中读出某个数值"的单步查读，需要包含定位 → 识别 → 读数 → 比较/计算 → 综合判断等多个步骤
   - **针对报表/表格**：不能是"直接读取表中某一格数值"，需要涉及多个单元格的计算、比例推算、跨行/跨列的综合分析等
...
5. **针对统计图表的特殊要求**：
   - **显式考察"读图能力"**（横轴、纵轴、图例）：
     ...
     * 使用图例/颜色/线型区分不同数据系列
   - **读图精度与提问方式**：
     * 如果数据点位于清晰的刻度或网格线位置，可要求精确数值并进行计算
     * 如果数据点处于刻度之间或超出范围，只允许设计比较或区间类型问题
     * 避免考察肉眼难以区分的极小差异
   - **多子图关联**：如果图片中存在多个子图，优先设计需要同时使用多个子图信息的链式推理题

6. **针对报表/表格的特殊要求**：
   - 需要跨行、跨列或跨表的数据整合
   - 涉及科目之间的勾稽关系验证
   - 需要理解会计科目的层级关系（如总科目与明细科目）
   - 需要进行比率、百分比、增长率等财务指标的计算

7. **思维链要求**：
   必须在 qa_make_process 字段中详细记录解题的完整推理链条，包括：
   - 从图像的哪些位置获取了哪些信息（具体的图例、坐标、表格单元格等）
   - 如何将这些信息组合起来进行推理
   - 每一步的计算或逻辑判断过程（包括具体公式和数值）
   - 最终如何得出答案

8. **严禁的行为**：
   - 不得编造图中不存在的图例名称、时间刻度、数值、单位、科目名称或表格内容
   - 不得引用图外正文、页眉页脚或其他图像外的信息
   - 不得在题干或过程里假设肉眼难以从图中可靠辨认的细节
   - 不得设计依赖外部知识或背景才能回答的问题

{"9. **多轮对话要求**：需要设计" + rounds_text + "轮对话，每轮都有独立的问题和答案，形成完整的对话流程。每轮问题都应遵循上述所有要求，且各轮之间应形成逻辑递进或关联（如：第1轮读取数据 → 第2轮计算指标 → 第3轮综合判断）。" if is_multi_round else ""}
""",
        "pure_text": f"""
...
{"7. **多轮对话要求**：需要设计" + rounds_text + "轮对话，每轮都有独立的问题和答案，形成完整的对话流程。每轮问题都应遵循上述所有要求，且各轮之间应形成逻辑递进或关联（如：第1轮识别数据 → 第2轮计算指标 → 第3轮综合判断）。" if is_multi_round else ""}
""",
        "splice": f"""
你是一名顶级的金融多模态高难度 VQA 题目构建专家，擅长从复杂的"图表+文字说明"类金融图像中，设计需要多层逻辑链条的专业级分析题。

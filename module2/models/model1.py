...
def extract_answer_with_fallback(text: str) -> tuple:
    ...
    if answer:
        print(f"[模型1] 从 \\boxed{{}} 中提取到答案: {answer[:50]}")
        ...
        if use_stream:
            ...
            if response.choices and len(response.choices) > 0:
                ...
                if hasattr(message, 'reasoning') and message.reasoning:
                    ...
                    print(f"[模型1] 检测到reasoning字段，长度: {len(reasoning_text)}")
                elif hasattr(message, 'reasoning_content') and message.reasoning_content:
                    reasoning_content = message.reasoning_content
                    print(f"[模型1] 检测到reasoning_content字段，长度: {len(reasoning_content)}")
                    ...
                    if isinstance(rd, list):
                        ...
                        if texts:
                            reasoning_text = "\n\n".join(texts)
                            print(f"[模型1] 检测到reasoning_details字段（列表），长度: {len(reasoning_text)}")
                    elif isinstance(rd, str) and rd.strip():
                        reasoning_text = rd.strip()
                        print(f"[模型1] 检测到reasoning_details字段（字符串），长度: {len(reasoning_text)}")
        
        # 提取答案和推理过程（使用改进的多层 fallback 机制）
        if full_content:
            # 按优先级使用：reasoning > reasoning_content
            if reasoning_text:
                process_content = reasoning_text
                print(f"[模型1] 使用reasoning/reasoning_details字段作为推理过程")

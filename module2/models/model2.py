...
def call_model2_api(question: str, image_path: Optional[str] = None) -> list:
    """
    调用模型2 API回答问题
    ...
    Returns:
        ...
        - process: 思考过程/推理过程
        - answer: 最终答案（已从answer_model2包裹中提取）
    ...
    try:
        # 从配置获取API信息
        api_config_name = MODEL_CONFIG["model2"]["name"]
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"[模型2] ⚠️ 速率限制错误（429），等待 {wait_time} 秒后重试（尝试 {attempt + 1}/{max_retries}）...")
                    time.sleep(wait_time)
                else:
                    # 最后一次重试失败，抛出异常
                    print(f"[模型2] ❌ 达到最大重试次数，仍然遇到速率限制错误")
                    raise
        ...
        print(f"[模型2] ✅ 调用完成，耗时: {response_time:.2f}秒")
        print(f"[模型2] process长度: {len(process_content)}, answer长度: {len(answer_content)}")
...
if __name__ == "__main__":
    ...
    print("测试模型2调用")
    print("=" * 60)
    result = call_model2_api(test_question)

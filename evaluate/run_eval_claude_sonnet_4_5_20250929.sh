...
INPUT_FILE="/nfsdata-117/project/DeepEyes_Benchmark/check12-29.csv" # 输入文件路径（支持 .json, .jsonl 或 .csv）
OUTPUT_FILE="check12-29.jsonl"                            # 只需要填写输出文件名，不要填写路径（支持 .json 或 .jsonl，根据扩展名自动判断格式）
...
MAX_RETRIES=2                                               # 请求失败时的最大重试次数，默认3次

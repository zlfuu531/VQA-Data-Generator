...
# ==============================================================================
# FinMME 数据集评测脚本
# 使用 evaluate_py 框架评测 FinMME 数据集
# ==============================================================================
set -eu
...
# ==============================================================================
# FinMME 本地数据集目录
FINMME_DATA_DIR="/nfsdata-117/project/finvlr1/Benchmark/FinMME_data"

# 图片保存目录
IMAGE_DIR="/nfsdata-117/project/finvlr1/Benchmark/FinMME_data/image"

# 转换后的数据文件（在 finmme 目录下）
CONVERTED_DATA_FILE="$SCRIPT_DIR/finmme_converted.jsonl"

# 输出和日志目录（都在 finmme 目录下）
LOG_DIR="$SCRIPT_DIR/logs"

# ==============================================================================
# 模型配置（固定为当前模型）
# ==============================================================================
EVAL_MODELS="qwen3-vl-235b-a22b-thinking"                                    # 当前脚本评测的模型：qwen3-vl-235b-a22b-thinking
...
LIMIT="3000"                                                   # 限制处理数量：设置为数字（如"10"）只处理前N条数据，设置为空字符串("")处理全部数据
...
# ==============================================================================
WORKERS=6                                               # 总并发线程数
BATCH=5                                                    # 批量处理大小，对应 EVAL_BATCH_SIZE
...
# ==============================================================================
TIMEOUT=1000                                                 # 单次API请求超时时间（秒），默认600秒
...
# 输出文件配置
# ==============================================================================
OUTPUT_FILE="finmme_eval_results.jsonl"                   # 只需要填写输出文件名，不要填写路径（支持 .json 或 .jsonl，根据扩展名自动判断格式）
...
# 数据转换
# ==============================================================================
print_info "步骤 1: 检查并转换 FinMME 数据..."

# 检查转换后的数据文件是否存在
if [ ! -f "$CONVERTED_DATA_FILE" ]; then
    print_info "转换后的数据文件不存在，开始转换..."
    # 设置离线模式环境变量，避免网络请求
    export HF_HUB_OFFLINE=1
    export HF_DATASETS_OFFLINE=1
    python "$SCRIPT_DIR/convert_data.py" \
        --local_dir "$FINMME_DATA_DIR" \
        --output "$CONVERTED_DATA_FILE" \
        --format jsonl \
        --image_dir "$IMAGE_DIR"
    print_success "数据转换完成: $CONVERTED_DATA_FILE"
else
    print_info "使用已存在的转换数据: $CONVERTED_DATA_FILE"
fi

# 检查输入文件是否存在
if [ ! -f "$CONVERTED_DATA_FILE" ]; then
    print_error "转换后的数据文件不存在: $CONVERTED_DATA_FILE"
    exit 1
fi
...
# 构建环境变量（传递给Python脚本）
# ==============================================================================
export EVAL_MODELS="qwen3-vl-235b-a22b-thinking"
...
# ==============================================================================
CMD_ARGS=(
    "--input_file" "$CONVERTED_DATA_FILE"
    "--log_dir" "$LOG_DIR"
    "--log_level" "$LOG_LEVEL"
...
echo "FinMME 数据集评测配置"
echo "=============================================================================="
echo "输入文件: $CONVERTED_DATA_FILE"
if [ -n "$OUTPUT_FILE" ]; then
    echo "输出文件: $OUTPUT_FILE (保存在 ./outputs/{profile}/{model_name}/ 目录下（/{profile}/{model_name}/ 目录下)"
else
    echo "输出文件: 自动生成（保存在 ./outputs/{profile}/{model_name}/ 目录下（/ 目录下，带时间戳）"
fi
echo "日志目录: $LOG_DIR"
echo "日志级别: $LOG_LEVEL"
echo ""

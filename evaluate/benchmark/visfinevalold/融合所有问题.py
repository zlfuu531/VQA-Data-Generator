...
# ===================== 题型数字前缀映射 =====================
# 参考 visfineval.py 中的 INDEX_PREFIX_MAP
INDEX_PREFIX_MAP = {
    '11': ('L1_Q1', '一级题型1'),  # 三选单选题 (A/B/C)
    '12': ('L1_Q2', '一级题型2'),  # 四选单选题 (A/B/C/D)
    '13': ('L1_Q3', '一级题型3'),  # 四选多选题 (A/B/C/D)
    '14': ('L1_Q4', '一级题型4'),  # 判断题 (无选项)
    '15': ('L1_Q5', '一级题型5'),  # 四选单选题 (A/B/C/D)
    '16': ('L1_Q6', '一级题型6'),  # 四选单选题 (A/B/C/D)
    '21': ('L2_Q1', '二级题型1'),  # 三选单选题 (A/B/C)
    '22': ('L2_Q2', '二级题型2'),  # 四选单选题 (A/B/C/D)
    '23': ('L2_Q3', '二级题型3'),  # 四选单选题 (A/B/C/D)
    '31': ('L3_Q1', '三级题型1(双模态)'),  # 四选单选题 (A/B/C/D)
    '32': ('L3_Q2', '三级题型2'),  # 四选单选题 (A/B/C/D)
    '33': ('L3_Q3', '三级题型3'),  # 四选单选题 (A/B/C/D)
    '34': ('L3_Q4', '三级题型4(多轮)')  # 多轮问答 (无选项)
}
...
def get_question_type(index: str) -> str:
    """
    根据index的前缀获取题型名称
    ...
    Returns:
        题型名称，如果无法匹配则返回空字符串
    """
    _, typename = get_question_type_info(index)
    return typename
...
def convert_row_to_excel_format(row: Dict[str, Any], scenario: str, tsv_filename: str = "") -> Dict[str, Any]:
    ...
    # 根据题型获取应该提取的选项列
    # 如果题型未知，默认提取所有选项（A/B/C/D）
    valid_options = QUESTION_TYPE_OPTIONS.get(type_key, ['A', 'B', 'C', 'D']) if type_key else ['A', 'B', 'C', 'D']
    
    # 只提取对应题型的选项列，其他选项列设为空字符串
    # 明确检查键是否存在，避免把answer列误当作D选项
    option_A = ""
    option_B = ""
    option_C = ""
    ...
    excel_row = {
        ...
        "option_C": option_C,
        "option_D": option_D,
        "answer": str(row.get('answer', '')).strip() if 'answer' in row and row.get('answer') is not None else "",
    }
...
def load_tsv_file(tsv_path: Path) -> List[Dict[str, Any]]:
    """
    加载TSV文件，使用pandas读取，参考visfineval.py的实现方式
    这样可以正确处理空列和列数不匹配的情况
    ...
    Returns:
        行数据列表
    """
    # 使用pandas读取TSV文件，dtype=str确保所有值都是字符串，fillna('')将空值填充为空字符串
    # 这样可以正确处理D列为空、answer为空等各种情况

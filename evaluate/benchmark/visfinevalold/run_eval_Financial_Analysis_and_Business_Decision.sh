#!/bin/bash
# ==============================================================================
# VisFinEval 数据集评测脚本 - Financial_Analysis_and_Business_Decision
# ==============================================================================
# 用途：使用 evaluate_py 框架评测 VisFinEval 数据集（Financial_Analysis_and_Business_Decision.jsonl）
# 说明：测试多个模型，所有配置项都在下面，直接修改即可
# ==============================================================================
set -eu
...
# ==============================================================================
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INPUT_FILE="$SCRIPT_DIR/Financial_Analysis_and_Business_Decision.jsonl"      # VisFinEval 数据文件
OUTPUT_FILE="Financial_Analysis_and_Business_Decision-0.3.jsonl"                              # 只需要填写输出文件名，不要填写路径（支持 .json 或 .jsonl，根据扩展名自动判断格式）
...
# ==============================================================================
# 从图片中获取的6个模型：
# 1. qwen3-vl-8b-thinking -> Qwen3-VL-8B-Thinking
# 2. qwen3-vl-8b-instruct -> Qwen3-VL-8B-Instruct
# 3. qwen3-vl-32b-thing -> qwen3-vl-32b-thinking (假设是thinking的typo)
# 4. qwen3-vl-235b-thinking -> qwen3-vl-235b-a22b-thinking
# 5. InternVL3_5-30B-A3B
# 6. InternVL3_5-241B-A28B
EVAL_MODELS="InternVL3_5-30B-A3B"
#Qwen3-VL-8B-Thinking,Qwen3-VL-8B-Instruct,qwen3-vl-32b-thinking,qwen3-vl-235b-a22b-thinking,,InternVL3_5-241B-A28B
# ==============================================================================
# 用户画像配置
# ==============================================================================
# 四种用户画像提示词：
...
LIMIT="1300"                                              #4273   限制处理数量：设置为数字（如"10"）只处理前N条数据，设置为空字符串("")处理全部数据
...
export EVAL_MULTI_ROUND_COUNT_BY_ROUNDS="$MULTI_ROUND_COUNT_BY_ROUNDS"  # 多轮题目是否按轮次计分
...
# ==============================================================================
echo "=============================================================================="
echo "评测配置 - Financial_Analysis_and_Business_Decision"
echo "=============================================================================="

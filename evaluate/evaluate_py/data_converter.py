...
def merge_multi_round_items(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for question_id, group_items in grouped.items():
        ...
        # 处理image_path（合并所有轮次的图片路径）
        all_image_paths = []
        for item in group_items:
            if "image_path" in item and item["image_path"]:
                if isinstance(item["image_path"], list):
                    all_image_paths.extend(item["image_path"])
                elif isinstance(item["image_path"], str):
                    # 可能是逗号分隔的字符串
                    if "," in item["image_path"]:
                        all_image_paths.extend([path.strip() for path in item["image_path"].split(",") if path.strip()])
                    else:
        ...
        if all_image_paths:
            ...
            merged_item["image_path"] = unique_paths if len(unique_paths) > 1 else (unique_paths[0] if unique_paths else "")
        else:
            # 如果没有合并到image_path，使用第一条的image_path
            merged_item["image_path"] = first_item.get("image_path", "")
        ...
        if all_image_urls:
            ...
            merged_item["image_urls"] = unique_urls
        else:
            merged_item["image_urls"] = first_item.get("image_urls", [])
        ...
        for idx, item in enumerate(group_items, 1):
            ...
            # 合并options为按round分组的字典格式
            if "options" in item and item["options"]:
                item_options = item["options"]
                # 如果item的options是字典格式，直接使用
                if isinstance(item_options, dict):
                    options_dict[round_key] = item_options
        ...
        # 如果合并到了options，使用合并后的字典格式；否则使用第一条数据的options（可能是None或单组选项）
        if options_dict:
            merged_item["options"] = options_dict
        else:
            # 如果没有按round分组的options，保留第一条数据的options（可能是None或单组选项）
            merged_item["options"] = first_item.get("options")
        
        # 保留其他字段（从第一条数据）
        for key, value in first_item.items():
            if key not in merged_item and key not in ["round", "question", "answer", "options", "image_path", "image_urls"]:
